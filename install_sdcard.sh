...
sed 's/nvme0n1/$dev/g' ~/tools/repair_device.sh >~/tools/repair_device_sdcard.sh
...
cmd echo "mount -o rw,remount / ; steamos-readonly disable; rm /usr/lib/udev/rules.d/99-sdcard-mount.rules" | steamos-chroot --disk /dev/$dev --partset A --

cmd echo "mount -o rw,remount / ; steamos-readonly disable; rm /usr/lib/udev/rules.d/99-sdcard-mount.rules" | steamos-chroot --disk /dev/$dev --partset B --
...
sudo mount /dev/$dev /run/media/home
